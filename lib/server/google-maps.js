...
function resolveStopPostojMinutes(stop, fallbackMinutes) {
  const parsed = Number(stop?.czas_postoju_min);
...
function buildEtaWindows({
  ...
  const etaStops = orderedStops.map((stop, index) => {
    ...
    const stopPostojMinutes = resolveStopPostojMinutes(
      stop,
      serviceDurationMinutes
    );
    ...
    cursor = new Date(
      actualEtaFrom.getTime() + stopPostojMinutes * 60 * 1000
    );
    ...
    return {
      ...
      czas_postoju_min: stopPostojMinutes,
    };
  });
...
export async function computeRoutePlan({
  ...
  const resolvedDestinationBase = destinationBase || base;
  const destinationLat = toNumber(resolvedDestinationBase?.lat);
  const destinationLon = toNumber(resolvedDestinationBase?.lon);
  ...
  if (destinationLat == null || destinationLon == null) {
    throw new Error("Brak poprawnie skonfigurowanego punktu koncowego trasy.");
  }
  ...
  const requestBody = {
    ...
    destination: toWaypoint(destinationLat, destinationLon),
