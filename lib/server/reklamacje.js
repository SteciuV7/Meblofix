...
const SMS_REJECTION_ALERT_ACKNOWLEDGED_ACTION =
  "route_sms_rejection_alert_acknowledged";
const SMS_REJECTION_ALERT_REJECTED_ACTION = "route_sms_confirmation_rejected";
...
async function loadLatestRouteStopsByComplaintIds(complaintIds = []) {
  const ids = [...new Set((complaintIds || []).filter(Boolean))];
  if (!ids.length) {
    return new Map();
  }
  ...
  const latestStops = new Map();

  for (const stop of data || []) {
    if (!latestStops.has(stop.reklamacja_id)) {
      latestStops.set(stop.reklamacja_id, stop);
    }
  }
...
async function loadSmsRejectionAlertStateByComplaintIds(complaintIds = []) {
  const ids = [...new Set((complaintIds || []).filter(Boolean))];
  if (!ids.length) {
    return new Map();
  }
  ...
  const { data, error } = await supabase
    ...
    .select("id,reklamacja_id,action,created_at,metadata")
    .in("reklamacja_id", ids)
    .in("action", [
      SMS_REJECTION_ALERT_REJECTED_ACTION,
      SMS_REJECTION_ALERT_ACKNOWLEDGED_ACTION,
    ])
    .order("created_at", { ascending: false });
  ...
  const alertStateByComplaintId = new Map();

  for (const log of data || []) {
    if (!log?.reklamacja_id) {
      continue;
    }

    const entry = alertStateByComplaintId.get(log.reklamacja_id) || {
      latestRejectedLog: null,
      latestAcknowledgedLog: null,
    };

    if (
      log.action === SMS_REJECTION_ALERT_REJECTED_ACTION &&
      !entry.latestRejectedLog
    ) {
      entry.latestRejectedLog = log;
    }

    if (
      log.action === SMS_REJECTION_ALERT_ACKNOWLEDGED_ACTION &&
      !entry.latestAcknowledgedLog
    ) {
      entry.latestAcknowledgedLog = log;
    }

    alertStateByComplaintId.set(log.reklamacja_id, entry);
  }

  return alertStateByComplaintId;
}
...
function formatDayWithoutTime(value) {
  if (!value) {
    return null;
  }
...
function buildSmsRejectionVisitLabel(routeStop, reklamacja = null) {
  const fromValue =
    routeStop?.eta_from ||
    routeStop?.etaFrom ||
    routeStop?.trasy?.data_trasy ||
    reklamacja?.trasa ||
    reklamacja?.realizacja_do ||
    null;
  const toValue = routeStop?.eta_to || routeStop?.etaTo || null;

  const fromDay = formatDayWithoutTime(fromValue);
  const toDay = formatDayWithoutTime(toValue);

  if (fromDay && toDay && fromDay !== toDay) {
    return {
      eventDate: `${fromDay}-${toDay}`,
      dayLabel: `w dniach ${fromDay}-${toDay}`,
    };
  }
...
function hasPendingSmsRejectionAlert(alertState) {
  const latestRejectedAt = alertState?.latestRejectedLog?.created_at;
  if (!latestRejectedAt) {
    return false;
  }

  const latestAcknowledgedAt = alertState?.latestAcknowledgedLog?.created_at;
  if (!latestAcknowledgedAt) {
    return true;
  }

  return new Date(latestRejectedAt) > new Date(latestAcknowledgedAt);
}

function buildPendingSmsRejectionAlert({
  reklamacja,
  routeStop = null,
  alertState = null,
}) {
  if (!hasPendingSmsRejectionAlert(alertState)) {
    return {
      hasAlert: false,
      message: null,
      eventDate: null,
  ...
  const customerName = [
    `${reklamacja?.imie_klienta || ""}`.trim(),
    `${reklamacja?.nazwisko_klienta || ""}`.trim(),
  ]
    .filter(Boolean)
    .join(" ");
  const customerLabel = customerName ? `Klient ${customerName}` : "Klient";
  const visitLabel = buildSmsRejectionVisitLabel(routeStop, reklamacja);

  return {
    hasAlert: true,
    message: visitLabel.dayLabel
      ? `${customerLabel} odmowil wizyty ${visitLabel.dayLabel}.`
      : `${customerLabel} odmowil wizyty.`,
    eventDate: visitLabel.eventDate,
    logId: alertState?.latestRejectedLog?.id || null,
  };
}
...
function buildPendingUserChanges({ reklamacja, logs = [] }) {
  ...
  const events = pendingLogs
    .map((log) => ({
      ...
      date: log.created_at,
      action: log.action,
      actionLabel: formatOperationalLogAction(log, { withTime: false }),
...
function stripTransientComplaintFields(payload = {}, actor = null) {
  ...
  delete nextPayload.addressGeocode;
...
function normalizeConfirmedAddressGeocode(payload = {}) {
  const geocode = payload?.addressGeocode;
...
export function normalizeComplaintClosePayload(payload = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(payload, "data_zakonczenia")) {
    normalized.data_zakonczenia = normalizeComplaintCompletionDateValue(
      payload.data_zakonczenia
    );
  }
...
export function normalizeComplaintCompletionDateValue(value) {
  if (value == null) {
    return null;
  }

  if (typeof value !== "string") {
    throw validationError("Data zakonczenia jest nieprawidlowa.");
  }
...
export async function listReklamacjeRecords({ actor }) {
  ...
  const complaintIds = reklamacje.map((item) => item.id);
  const [activeRouteStops, latestRouteStops, smsRejectionAlerts] =
    await Promise.all([
      loadActiveRouteStopsByComplaintIds(complaintIds),
      actor.role !== ROLE.ADMIN
        ? loadLatestRouteStopsByComplaintIds(complaintIds)
        : Promise.resolve(new Map()),
      actor.role !== ROLE.ADMIN
        ? loadSmsRejectionAlertStateByComplaintIds(complaintIds)
        : Promise.resolve(new Map()),
    ]);
  ...
  return reklamacje.map((reklamacja) => ({
    ...
    hasPendingSmsRejectionAlert:
      actor.role !== ROLE.ADMIN &&
      buildPendingSmsRejectionAlert({
        reklamacja,
        routeStop: latestRouteStops.get(reklamacja.id) || null,
        alertState: smsRejectionAlerts.get(reklamacja.id) || null,
      }).hasAlert,
  }));
}
...
export async function getReklamacjaDetail({ reklamacjaId, actor }) {
  ...
  const pendingSmsRejectionAlert =
    actor.role !== ROLE.ADMIN
      ? buildPendingSmsRejectionAlert({
          reklamacja,
          routeStop: routeStops?.[0] || null,
          alertState: {
            latestRejectedLog: (logs || []).find(
              (log) => log.action === SMS_REJECTION_ALERT_REJECTED_ACTION
            ),
            latestAcknowledgedLog: (logs || []).find(
              (log) =>
                log.action === SMS_REJECTION_ALERT_ACKNOWLEDGED_ACTION
            ),
          },
...
export async function updateReklamacjaRecord({
  ...
  const supabase = getSupabaseAdmin();
  const addressApprovalMode =
    payload?.addressApprovalMode === "approximate" ? "approximate" : "exact";
  const confirmedAddressGeocode = normalizeConfirmedAddressGeocode(payload);
  ...
  if (addressChanged) {
    ...
    coords =
      confirmedAddressGeocode ||
      (await resolveComplaintGeocode(sanitized, {
        allowApproximate: addressApprovalMode === "approximate",
        optional: false,
      }));
  }
...
export async function manuallyChangeComplaintStatus({
  ...
  if (nextStatus === REKLAMACJA_STATUS.DONE) {
    const normalizedClosePayload = validateComplaintClosePayload(closePayload);

    patch = {
      ...normalizeComplaintInfoPatch(closePayload),
      ...normalizedClosePayload,
      data_zakonczenia:
        normalizedClosePayload.data_zakonczenia || new Date().toISOString(),
    };
...
export async function acknowledgeComplaintSmsRejectionAlert({
  reklamacjaId,
  actor,
}) {
  const reklamacja = await loadComplaintById(reklamacjaId);
  ...
  await createOperationalLog({
    entity_type: "reklamacja",
    entity_id: reklamacjaId,
    reklamacja_id: reklamacjaId,
    action: SMS_REJECTION_ALERT_ACKNOWLEDGED_ACTION,
    before_state: null,
    after_state: null,
    metadata: {
