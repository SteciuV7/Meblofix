...
function getRouteBaseAddress(route, settings) {
  const snapshotLat = parseBaseCoordinate(route?.base_lat_snapshot);
  const snapshotLon = parseBaseCoordinate(route?.base_lon_snapshot);
  const snapshotAddress =
    typeof route?.base_address_snapshot === "string"
      ? route.base_address_snapshot.trim()
      : "";
...
export async function getRouteDetail(routeId) {
  ...
  const routeBase = getRouteBaseAddress(route, settings);
  ...
  const mapPreview = buildRouteMapPreview(stops, route, routeBase);
...
export async function computeRoutePreview({
  ...
  const routeBase = resolveRouteBaseForPlanning(
    settings,
  ...
  return {
    settings,
    base: {
      adres_bazy: routeBase.adres_bazy || null,
      lat: routeBase.lat ?? null,
      lon: routeBase.lon ?? null,
      source: routeBase.source,
      matchType: routeBase.matchType,
    },
    ...planned,
  };
}
...
export async function createRoute({
  ...
  const rpcPayload = {
    p_route: {
      ...
      base_address_snapshot: preview.base?.adres_bazy || preview.settings.adres_bazy,
      base_lat_snapshot: preview.base?.lat ?? null,
      base_lon_snapshot: preview.base?.lon ?? null,
