...
import { createOperationalLog, validateComplaintClosePayload } from "@/lib/server/reklamacje";
import {
...
import { getComplaintCustomerName, getPhoneHref, normalizePolishPhoneNumber } from "@/lib/utils";
...
function buildSmsStateSnapshot(stop) {
  return {
    ...
    sms_potwierdzenie_confirmed_at: stop?.sms_potwierdzenie_confirmed_at || null,
...
function ensurePlannedRoute(route, message = "SMS potwierdzen mozna wysylac tylko dla zaplanowanej trasy.") {
...
function ensureSmsManualInteractionAllowed(route) {
  if (![ROUTE_STATUS.PLANNED, ROUTE_STATUS.IN_PROGRESS].includes(route?.status)) {
...
function getRouteBaseAddress(route, settings) {
  return settings || (route.base_address_snapshot ? { adres_bazy: route.base_address_snapshot } : null);
}

function buildRouteMapPreview(stops, route, settings) {
  return {
    shouldCompute: Boolean(settings && (stops || []).length),
    stops: (stops || []).map((stop) => stop.reklamacje).filter(Boolean),
...
async function resetStopSmsConfirmationState(stop, actor, reason) {
  ...
  const updatedStop = await updateRouteStopSmsState(stop.id, defaultSmsStatePatch());
...
function buildPublicConfirmationPayload(stop, settings) {
  return {
    ...
    complaintNumber:
      stop?.reklamacje?.nr_reklamacji || stop?.reklamacje?.numer_faktury || null,
    address: [
...
export async function recalculateRoute({
  ...
  const previousStatusMap = new Map(
    previousDetail.stops.map((stop) => [stop.reklamacja_id, stop.previous_reklamacja_status])
  );
  ...
  if (resetSmsConfirmations) {
    ...
    for (const previousStop of previousDetail.stops) {
      ...
      await updateRouteStopSmsState(refreshedStop.id, {
        ...
        sms_potwierdzenie_sent_at: previousStop.sms_potwierdzenie_sent_at || null,
...
export async function sendRouteConfirmationSmsBatch(routeId, actor) {
  ...
  if (detail.route.smsConfirmationsSentAt) {
    throw createHttpError(
      "Zbiorcze SMS potwierdzen zostaly juz wyslane. Edytuj trase z resetem, aby wyslac je ponownie.",
      409
    );
  }
  ...
  for (const stop of detail.stops) {
    try {
      ...
      failedCount += 1;
      warnings.push(
        error.message || `Nie udalo sie wyslac SMS dla ${buildStopWarningLabel(stop)}.`
      );
    }
  }
  ...
  const beforeState = {
    sms_potwierdzenia_wyslane_at: detail.route.sms_potwierdzenia_wyslane_at || null,
  };

  if (sentCount > 0) {
    route = await updateRouteSmsBatchTimestamp(routeId, new Date().toISOString());
    ...
    await createRouteLog({
      ...
      beforeState,
      afterState: {
        sms_potwierdzenia_wyslane_at: route.sms_potwierdzenia_wyslane_at || null,
      },
      metadata: {
        total_count: detail.stops.length,
  ...
  return {
    ...
    sentCount,
    failedCount,
    totalCount: detail.stops.length,
    warnings,
  };
}
...
export async function logPublicSmsConfirmationCall(token) {
  ...
  if (!settings?.sms_kontakt_telefon) {
    throw createHttpError("Brak numeru telefonu kontaktowego w ustawieniach.", 422);
  }
...
export async function startRoute(routeId, actor) {
  ...
  try {
    ...
    warnings.push(
      sendError.message || "Trasa wystartowala, ale nie udalo sie uruchomic wysylki SMS."
    );
  }
...
export async function undeliverRouteStop(routeId, stopId, actor) {
  const supabase = getSupabaseAdmin();
