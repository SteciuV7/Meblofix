...
function parseRequiredText(value, label) {
  ...
  if (!normalized) {
    throw createHttpError(`${label} jest wymagane.`, 422);
  }
...
function parseDecimal(value, label, { min, max } = {}) {
  ...
  if (!Number.isFinite(parsed)) {
    throw createHttpError(`${label} musi byc liczba.`, 422);
  }

  if (min != null && parsed < min) {
    throw createHttpError(`${label} nie moze byc mniejsze niz ${min}.`, 422);
  }

  if (max != null && parsed > max) {
    throw createHttpError(`${label} nie moze byc wieksze niz ${max}.`, 422);
  }

  return parsed;
}

function parseBaseCoordinate(value, { min, max } = {}) {
  if (value == null) {
    return null;
  }
...
function normalizeRouteStartPointPreviewPayload(payload = {}) {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    throw createHttpError("Nieprawidlowe dane punktu startu trasy.", 422);
  }

  return {
    addressLine: parseRequiredText(payload.adres, "Adres"),
    town: parseRequiredText(payload.miejscowosc, "Miejscowosc"),
    postalCode: parseRequiredText(payload.kod_pocztowy, "Kod pocztowy"),
  };
}

function normalizeRouteStartPointOverride(rawOverride) {
  if (rawOverride == null) {
    return null;
  }
  ...
  const matchType = `${rawOverride.matchType ?? ""}`.trim();
  if (!["exact", "approximate"].includes(matchType)) {
    throw createHttpError("Wybrany tryb potwierdzenia adresu jest nieprawidlowy.", 422);
  }

  return {
    adres_bazy: parseRequiredText(rawOverride.address, "Adres punktu startu"),
    lat: parseDecimal(rawOverride.lat, "Szerokosc geograficzna punktu startu", {
      min: -90,
      max: 90,
    }),
    lon: parseDecimal(rawOverride.lon, "Dlugosc geograficzna punktu startu", {
      min: -180,
      max: 180,
    }),
    matchType,
...
function resolveRouteBaseForPlanning(settings, startPointOverride = null) {
  ...
  return {
    ...settings,
    adres_bazy: startPointOverride.adres_bazy,
    lat: startPointOverride.lat,
    lon: startPointOverride.lon,
    source: "override",
    matchType: startPointOverride.matchType,
  };
}

function resolveRouteReturnBaseForPlanning(settings) {
  return {
    ...settings,
    source: "settings",
    matchType: "exact",
...
function serializeRouteBase(base = null) {
  return {
    adres_bazy: base?.adres_bazy || null,
    lat: base?.lat ?? null,
    lon: base?.lon ?? null,
...
async function syncComplaintDeadlineFromConfirmedSms({
  stop,
  actor = null,
  actorPayload = null,
  source,
  ...
  if (
    getStopSmsStatus(stop) !== SMS_CONFIRMATION_STATUS.CONFIRMED ||
    !stop?.reklamacja_id ||
    !stop?.eta_from
  ) {
    return null;
  ...
  const nextDeadline = normalizeDateForComparison(stop.eta_from);
  if (!nextDeadline) {
    return null;
  }

  const previousComplaint = stop.reklamacje || null;
  if (
    normalizeDateForComparison(previousComplaint?.realizacja_do) ===
    nextDeadline
  ) {
  ...
  const { data, error } = await supabase
    .from("reklamacje")
    .update({
      realizacja_do: nextDeadline,
      nieprzeczytane_dla_uzytkownika: true,
    })
    .eq("id", stop.reklamacja_id)
    .select("*")
    .single();
  ...
  await createOperationalLog({
    ...
    trasa_punkt_id: stop.id || null,
    action: "reklamacja_realizacja_do_updated_from_sms_confirmation",
    before_state: previousComplaint,
    after_state: data,
    metadata: {
      source,
      eta_from: nextDeadline,
      previous_realizacja_do: previousComplaint?.realizacja_do || null,
      next_realizacja_do: data.realizacja_do,
      trasa_numer: stop.trasy?.numer || null,
      kolejnosc: stop.kolejnosc || null,
    },
...
function ensureSmsManualInteractionAllowed(route) {
  if (route?.status !== ROUTE_STATUS.PLANNED) {
    throw createHttpError(
      "Lampke SMS mozna zmieniac tylko dla trasy zaplanowanej.",
      409,
    );
  }
}
...
function canStopBeConfirmedByCustomer(stop) {
  if (!stop?.trasy || !stop?.id) {
    return false;
  }

  if (stop.trasy.status !== ROUTE_STATUS.PLANNED) {
    return false;
  }

  if (ROUTE_STOP_FINAL_STATUSES.includes(stop.status)) {
    return false;
  }

  return getStopSmsStatus(stop) !== SMS_CONFIRMATION_STATUS.CONFIRMED;
}

function normalizeSnapshotAddress(value) {
  return typeof value === "string" ? value.trim() : "";
}

function buildRouteBaseFromSnapshot({
  settings,
  snapshotAddress,
  snapshotLat,
  snapshotLon,
}) {
  const hasSnapshotCoordinates = snapshotLat != null && snapshotLon != null;
  const hasSnapshotAddress = Boolean(snapshotAddress);

  if (!hasSnapshotCoordinates && !hasSnapshotAddress) {
    return null;
  }
  ...
  return {
    ...(settings || {}),
    ...(hasSnapshotAddress
      ? { adres_bazy: snapshotAddress }
      : settings?.adres_bazy
        ? { adres_bazy: settings.adres_bazy }
        : {}),
    ...(hasSnapshotCoordinates
      ? { lat: snapshotLat, lon: snapshotLon }
...
function getRouteLegacyBaseSnapshot(route, settings) {
  return buildRouteBaseFromSnapshot({
    settings,
    snapshotAddress: normalizeSnapshotAddress(route?.base_address_snapshot),
    snapshotLat: parseBaseCoordinate(route?.base_lat_snapshot, {
      min: -90,
      max: 90,
    }),
    snapshotLon: parseBaseCoordinate(route?.base_lon_snapshot, {
      min: -180,
      max: 180,
    }),
  });
...
function getRouteStartBase(route, settings) {
  const legacyBaseSnapshot = getRouteLegacyBaseSnapshot(route, settings);
  if (legacyBaseSnapshot) {
    return legacyBaseSnapshot;
  }

  return settings || null;
}

function getRouteReturnBase(route, settings, fallbackStartBase = null) {
  const returnBaseSnapshot = buildRouteBaseFromSnapshot({
    settings,
    snapshotAddress: normalizeSnapshotAddress(
      route?.return_base_address_snapshot,
    ),
    snapshotLat: parseBaseCoordinate(route?.return_base_lat_snapshot, {
      min: -90,
      max: 90,
    }),
    snapshotLon: parseBaseCoordinate(route?.return_base_lon_snapshot, {
      min: -180,
      max: 180,
    }),
  });

  if (returnBaseSnapshot) {
    return returnBaseSnapshot;
  }
  ...
  return settings || fallbackStartBase || null;
}

function buildRouteMapPreview({
  stops = [],
  route,
  settings,
  startBase,
  ...
  const resolvedStartBase = startBase || settings || null;
  const resolvedReturnBase = returnBase || resolvedStartBase || settings || null;

  return {
    shouldCompute: Boolean(resolvedStartBase && stops.length),
    base: resolvedStartBase,
    destinationBase: resolvedReturnBase,
    stops: stops
      .map((stop) => {
        if (!stop?.reklamacje) {
          return null;
...
function buildPublicConfirmationPayload(stop, settings) {
  const canConfirm = canStopBeConfirmedByCustomer(stop);
  const routeStatus = stop?.trasy?.status || null;
  const status = getStopSmsStatus(stop);
  const isConfirmed = status === SMS_CONFIRMATION_STATUS.CONFIRMED;
  const isRejected = status === SMS_CONFIRMATION_STATUS.MANUAL_REJECTED;
  const isRouteLocked = routeStatus !== ROUTE_STATUS.PLANNED;

  const confirmationBlockedReason =
    canConfirm || isConfirmed
      ? null
      : isRouteLocked
        ? "Trasa jest w trakcie realizacji. Potwierdzenie online jest juz niedostepne."
        : "Potwierdzenie online jest niedostepne.";

  return {
    status,
    canReject: !isRouteLocked && !ROUTE_STOP_FINAL_STATUSES.includes(stop?.status),
...
export async function getRouteDetail(routeId) {
  ...
  const routeStartBase = getRouteStartBase(route, settings);
  const routeReturnBase = getRouteReturnBase(route, settings, routeStartBase);
  ...
  const mapPreview = buildRouteMapPreview({
    stops,
    route,
    settings,
    startBase: routeStartBase,
    returnBase: routeReturnBase,
  });

  if (mapPreview.shouldCompute) {
    try {
      const planned = await computeRoutePlan({
        base: mapPreview.base,
        destinationBase: mapPreview.destinationBase,
  ...
  return {
    route,
    mapStartBase: routeStartBase,
    mapReturnBase: routeReturnBase,
    mapBase: routeStartBase,
    encodedPolyline,
...
export async function computeRoutePreview({
  ...
  startBaseOverride = null,
  returnBaseOverride = null,
}) {
  const supabase = getSupabaseAdmin();
  const settings = await getOperationalSettings({ required: true });
  const normalizedStartPointOverride =
    normalizeRouteStartPointOverride(startPointOverride);
  const startBase =
    startBaseOverride ||
    resolveRouteBaseForPlanning(settings, normalizedStartPointOverride);
  const returnBase = returnBaseOverride || resolveRouteReturnBaseForPlanning(settings);
  const { data: complaints, error } = await supabase
    .from("reklamacje")
    .select("*")
    .in("id", reklamacjeIds);
  ...
  return {
    settings,
    startBase: serializeRouteBase(startBase),
    returnBase: serializeRouteBase(returnBase),
    base: serializeRouteBase(startBase),
    ...planned,
  };
}
...
export async function createRoute({
  ...
  const startBaseSnapshot = preview.startBase || preview.base || preview.settings;
  const returnBaseSnapshot = preview.returnBase || preview.settings || preview.base;
  ...
  const rpcPayload = {
    p_route: {
      ...
      base_address_snapshot:
        startBaseSnapshot?.adres_bazy || preview.settings.adres_bazy,
      base_lat_snapshot: startBaseSnapshot?.lat ?? null,
      base_lon_snapshot: startBaseSnapshot?.lon ?? null,
      return_base_address_snapshot:
        returnBaseSnapshot?.adres_bazy || preview.settings.adres_bazy,
      return_base_lat_snapshot: returnBaseSnapshot?.lat ?? null,
      return_base_lon_snapshot: returnBaseSnapshot?.lon ?? null,
...
export async function previewRouteStartPoint(payload = {}) {
  const requestedAddress = normalizeRouteStartPointPreviewPayload(payload);
  const geocode = await previewGeocodeAddressServer(requestedAddress);
...
export async function recalculateRoute({
  ...
  const preview = await computeRoutePreview({
    ...
    startBaseOverride: previousDetail.mapStartBase || null,
    returnBaseOverride: previousDetail.mapReturnBase || null,
  });
...
export async function cancelPlannedRoute({ routeId, actor }) {
  const route = await loadRouteRecord(routeId);
  ensurePlannedRoute(
    route,
    "Usuniecie jest dozwolone tylko dla tras zaplanowanych.",
  );

  const supabase = getSupabaseAdmin();
  const { data, error } = await supabase.rpc("cancel_planned_route", {
    p_route_id: routeId,
    ...actorToRpcPayload(actor),
  });
...
export async function updateRouteStopSmsConfirmationStatus({
  ...
  await syncComplaintDeadlineFromConfirmedSms({
    stop: updatedStop,
    actor,
...
export async function confirmPublicSmsConfirmation(token) {
  ...
  if (getStopSmsStatus(stop) !== SMS_CONFIRMATION_STATUS.CONFIRMED) {
    if (!canStopBeConfirmedByCustomer(stop)) {
      throw createHttpError(
        "Potwierdzenie online jest juz niedostepne dla tej trasy.",
        409,
      );
    }
...
export async function rejectPublicSmsConfirmation(token) {
  const stop = await loadRouteStopByToken(token);
  ensurePublicTokenIsActive(stop);

  let updatedStop = stop;
  if (getStopSmsStatus(stop) !== SMS_CONFIRMATION_STATUS.MANUAL_REJECTED) {
    updatedStop = await updateRouteStopSmsState(stop.id, {
      sms_potwierdzenie_status: SMS_CONFIRMATION_STATUS.MANUAL_REJECTED,
      sms_potwierdzenie_confirmed_at: null,
    });

    await createComplaintRouteLog({
      stop: updatedStop,
      action: "route_sms_confirmation_rejected",
      actorPayload: {
        actor_firma_id: null,
        actor_email: "public-link",
        actor_role: "public",
...
export async function deliverRouteStop(routeId, stopId, actor, payload = {}) {
  ...
  const informacje = normalizeComplaintInfoValue(payload);
  const { data, error } = await supabase.rpc("deliver_route_stop", {
    p_route_id: routeId,
    p_stop_id: stopId,
    p_data_zakonczenia: closePayload.data_zakonczenia,
    ...
    ...(informacje !== undefined ? { p_informacje: informacje } : {}),
    ...actorToRpcPayload(actor),
  });
...
export async function undeliverRouteStop(routeId, stopId, actor, payload = {}) {
  ...
  const { data, error } = await supabase.rpc("undeliver_route_stop", {
    ...
    p_opis_przebiegu: undeliverPayload.opis_przebiegu,
    p_zalacznik_pdf_zakonczenie: undeliverPayload.zalacznik_pdf_zakonczenie,
    p_zalacznik_zakonczenie: undeliverPayload.zalacznik_zakonczenie,
    ...actorToRpcPayload(actor),
  });
