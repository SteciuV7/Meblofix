...
export default function Users() {
  ...
  useEffect(() => {
    async function fetchUsers() {
      const { data, error } = await supabase.from("firmy").select("firma_id, nazwa_firmy, email, rola");
      if (error) {
  ...
  return (
    <div className="relative min-h-screen bg-gray-100">
      ...
      <div className="max-w-4xl mx-auto py-10">
        <h2 className="text-3xl font-bold text-gray-900 mb-6">Zarządzaj użytkownikami systemu</h2>
              ...
              {users.length > 0 ? (
                users.map((user, index) => (
                  <tr key={index} className="border-t">
                    ...
                    onClick={() => {
                    console.log("🟢 Kliknięto 'Usuń' dla użytkownika:", user);
      ...
      {isOpen && (
        <div className="fixed inset-0 flex justify-end z-50">
          ...
          <div className="relative w-1/3 bg-white shadow-lg p-6 rounded-l-lg z-50">
            <h2 className="text-xl font-bold text-gray-900">Dodaj użytkownika</h2>
...
/>
            ...
            <div className="flex justify-end mt-6 space-x-2">
              <button className="bg-gray-700 text-white px-4 py-2 rounded-md" onClick={closeModal}>
                Zamknij
              </button>
              <button 
  className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700" 
...
{confirmDelete && userToDelete && (
  <div className="fixed inset-0 flex justify-center items-center z-50">
    ...
    <div className="bg-white p-6 rounded-lg shadow-lg z-50">
      ...
      <p className="text-gray-900">Czy na pewno chcesz usunąć użytkownika <b>{userToDelete?.email}</b>?</p>
      <div className="flex justify-end space-x-4 mt-4">
        <button className="bg-gray-500 text-white px-4 py-2 rounded-md" onClick={() => setConfirmDelete(false)}>
          Anuluj
        </button>
        <button className="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700" onClick={deleteUser}>
          Usuń
        </button>
      </div>
    </div>
...
{isEditOpen && userToEdit && (
  <div className="fixed inset-0 flex justify-center items-center z-50">
    ...
    <div className="bg-white p-6 rounded-lg shadow-lg z-50 w-1/3">
      <h2 className="text-lg font-bold text-gray-900">Edytuj użytkownika: {userToEdit.email}</h2>
      ...
      <label className="block text-gray-900 mt-3">Nowe Hasło (opcjonalnie)</label>
      <input
        className="w-full p-2 border rounded-md text-gray-900"
        type="text"
        value={newPassword}
      ...
      <div className="flex justify-end space-x-2 mt-4">
        <button className="bg-gray-500 text-white px-4 py-2 rounded-md" onClick={() => setIsEditOpen(false)}>
          Anuluj
        </button>
        <button className="bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700" onClick={updateUser}>
          Zapisz zmiany
        </button>
      </div>
    </div>
