create sequence if not exists public.trasy_numer_seq
  as bigint
  increment by 1
  minvalue 1
  start with 1;
...
as $$
  select 'T' || lpad(nextval('public.trasy_numer_seq')::text, 3, '0');
$$;
...
declare
  v_max integer;
begin
  select max((substring(numer from '^T([0-9]+)$'))::integer)
  into v_max
  from public.trasy
  where numer ~ '^T[0-9]+$';

  if v_max is null then
    perform setval('public.trasy_numer_seq', 1, false);
  else
    perform setval('public.trasy_numer_seq', v_max, true);
  end if;
end;
$$;

alter table if exists public.trasy
alter column numer set default public.generate_route_number();
