drop function if exists public.deliver_route_stop(uuid, uuid, text, text, jsonb, uuid, text, text);
drop function if exists public.undeliver_route_stop(uuid, uuid, text, uuid, text, text);
...
create function public.undeliver_route_stop(
  p_route_id uuid,
  p_stop_id uuid,
  p_informacje text default null,
  p_opis_przebiegu text default null,
...
begin
  ...
  update public.reklamacje
  set
    status = U&'Oczekuje na dostaw\0119',
    data_zakonczenia = null,
    informacje = nullif(trim(coalesce(p_informacje, '')), ''),
    opis_przebiegu = nullif(trim(coalesce(p_opis_przebiegu, '')), ''),
    zalacznik_pdf_zakonczenie = p_zalacznik_pdf_zakonczenie,
    zalacznik_zakonczenie = array(
      select jsonb_array_elements_text(coalesce(p_zalacznik_zakonczenie, '[]'::jsonb))
